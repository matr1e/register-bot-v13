...
exports.run = async (client, message, args) => {
  ...
  function embed(msg, decision, time) {
    ...
    if(sec === "tagRole"){
      if(!cr) return embed(`Bir rol etiketlemelisiniz!`);
      await botReq.findOneAndUpdate({ guildId: message.guild.id }, { $set: { TagRole: cr.id } }, { upsert: true });
      embed(`Başarıyla <@&${cr.id}> adlı rol "Tag Rol" olarak ayarlandı!`, "green", 10);
    }
    ...
    if(sec === "welcomeCh"){
      if(!cr) return embed(`Bir kanal etiketlemelisiniz!`);
      await botReq.findOneAndUpdate({ guildId: message.guild.id }, { $set: { WelcomeChannel: cr.id } }, { upsert: true });
      embed(`Başarıyla <#${cr.id}> adlı rol "Welcome Channel" olarak ayarlandı!`, "green", 10);
    }
    ...
    if(sec === "tagLogCh"){
      if(!cr) return embed(`Bir kanal etiketlemelisiniz!`);
      await botReq.findOneAndUpdate({ guildId: message.guild.id }, { $set: { TagLog: cr.id } }, { upsert: true });
      embed(`Başarıyla <#${cr.id}> adlı rol "Tag Log" olarak ayarlandı!`, "green", 10);
    }
